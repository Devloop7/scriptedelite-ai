...
from PySide6.QtWidgets import (
    ...
    QRadioButton, QToolTip, QSplitter, QMessageBox,
)
...
from core.tracker import StickyTargetTracker
...
class AimWorker(QObject):
    ...
    def __init__(self, cfg: AppConfig):
        ...
        self.tracker = StickyTargetTracker(
            max_miss_frames=12,
            match_iou=0.12,
            match_center_px=140.0,
            aim_smooth_far=0.40,
            aim_smooth_near=0.80,
            near_px=45.0,
        )
        self.running = False
        ...
        while self.running:
            ...
            try:
                ...
                if self.cfg.engine in ("yolo", "hybrid") and self.yolo:
                    ...
                    # Sticky track: same player until lost (not re-pick every frame)
                    ...
                    if best and self.cfg.use_linear_prediction and not best.get("coasting"):
                        ...
                        if pred:
                            # Blend prediction lightly so it can't yank lock off head
                            px, py = pred
                            best["aim_x"] = best["aim_x"] * 0.65 + px * 0.35
                            best["aim_y"] = best["aim_y"] * 0.65 + py * 0.35
                ...
                # ── Color fallback (only if no YOLO sticky track) ──
                if self.cfg.engine in ("color", "hybrid") and best is None:
                    ...
                    if ch:
                        ...
                        if dist < max_dist:
                            best = {
                                "aim_x": ax, "aim_y": ay, "dx": dx, "dy": dy,
                                "dist": dist, "sticky": False, "coasting": False,
                            }
                ...
                if should_aim and best:
                    ...
                    # Only full reset when not aiming — keep sticky track identity
                    # so re-pressing activation continues the same player.
                    self.controller.reset()
                    if not should_aim and not best:
                        self.tracker.reset()
                        self.predictor.reset()
                    elif not best:
                        self.predictor.reset()
...
class MainWindow(QMainWindow):
    ...
    def _tab_model(self) -> QWidget:
        ...
        gl = QVBoxLayout(g)
        gl.addWidget(_hint(
            "YOLO — neural person detection (recommended). Uses assets/model.pt.\n"
            "Color — fast magenta/green nameplate scan (fallback).\n"
            "Hybrid — YOLO first, color if nothing found."
        ))
        ...
        g3 = QGroupBox("COLOR DETECTION SETTINGS")
        g3l = QFormLayout(g3)
        ...
        g3l.addRow(_hint("Only used when Engine is Color or Hybrid (color path)."))
        lay.addWidget(g3)
        lay.addStretch()
        return w
    ...
    def on_config_changed(self, *_):
        ...
        if self.worker:
            self.worker.cfg = c
            # Keep predictor horizon in sync
