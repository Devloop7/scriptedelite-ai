        ...
        while self.running:
            ...
            try:
                ...
                if self.cfg.engine in ("yolo", "hybrid") and self.yolo:
                    ...
                    zone = max(self.cfg.zone_radius, 1)
                    best = self.yolo.find_best_target(
                        dets,
                        (screen_cx, screen_cy),
                        min(self.cfg.max_distance, zone) if self.cfg.zone_radius else self.cfg.max_distance,
                        offset,
                    ...
                    if best and self.cfg.use_linear_prediction:
                        ...
                        if pred:
                            ...
                            cv2.circle(vis, (int(pred[0]), int(pred[1])), 4, (255, 180, 0), -1)
                    elif not best:
                        self.predictor.reset()
                else:
                    vis = frame.copy()
                ...
                if self.cfg.engine in ("color", "hybrid") and (best is None or self.cfg.engine == "color"):
                    ...
                    if ch:
                        ...
                        zone = max(self.cfg.zone_radius, 1)
                        if dist < min(self.cfg.max_distance, zone):
                ...
                if best and best.get("dist", 9999) > zone_r:
                    best = None
                ...
                if should_aim and best:
                    ...
                    if self.cfg.shake_reduction:
                        # Extra mild smoothing when shake reduction is on
                        smooth = min(0.92, smooth + 0.08)
                    self.controller.move_to_target(
                        best["dx"], best["dy"],
                        smoothing=smooth,
                        strength=self.cfg.strength,
                        speed=self.cfg.acquisition_speed / 3.0,  # normalize ~1.0 around speed 3
...
class MainWindow(QMainWindow):
    ...
    def on_frame(self, frame, stats: dict):
        self.stats_lab.setText(
            ...
            f"Detections: {stats.get('dets', 0)}   "
            f"Target: {'YES' if stats.get('target') else 'no'}   "
            f"Activation: {'HELD' if stats.get('activation') else '—'}   "
            f"Pad: {'yes' if stats.get('pad') else 'no'}   "
            f"Res: {stats.get('res', '?')}   "
            f"Eng: {stats.get('engine', '?')}"
        )
